...
export default function AbonarClienteBtn({ customerId, nombreCliente, saldoActual }: Props) {
  ...
  async function registrar() {
    ...
    const { error: errAbono } = await supabase.from('customer_credit_payments').insert({
      customer_id: customerId,
      monto: montoNum,
      metodo_pago: metodo,
      nota: nota.trim() || `Abono deuda — ${nombreCliente}`,
      fecha: new Date().toISOString().split('T')[0],
      usuario_id: user?.id ?? null,
    })
    if (errAbono) { toast.error('Error al registrar abono: ' + errAbono.message); setSaving(false); return }

    const nuevoSaldo = Math.max(0, saldoActual - montoNum)
    await supabase.from('customers').update({ saldo_deudor: nuevoSaldo }).eq('id', customerId)

    toast.success(`Abono de ${formatCLP(montoNum)} registrado a ${nombreCliente}`)
    setOpen(false)
    setNota('')
    router.refresh()
    setSaving(false)
  ...
  return (
    <>
      <button
        ...
        className="text-xs px-2.5 py-1.5 bg-green-50 text-green-700 border border-green-300 rounded-lg font-semibold hover:bg-green-100 transition-colors whitespace-nowrap"
      >
        💰 Registrar abono
      </button>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="Ej: Pago parcial venta V-00000010"
                  className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
                />
              </div>
            </div>
