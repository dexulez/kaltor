...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase.from('sales').select('total, iva, ppm')
      .gte('created_at', hoyStart).lte('created_at', hoyEnd).eq('anulada', false),
    ...
    supabase.from('sales')
      ...
      .gte('created_at', hoyStart).lte('created_at', hoyEnd)
  ...
  return (
    <div className="p-4 sm:p-6 space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-gray-800 text-sm">💰 Ventas de hoy</h2>
            <Link href="/caja" className="text-xs text-blue-600 hover:underline">Ver caja →</Link>
          </div>
          <div className="bg-white rounded-xl border overflow-hidden">
            {!ventasList.length ? (
              <div className="text-center py-10 text-sm text-gray-400">Sin ventas hoy</div>
            ) : (
              <div className="divide-y">
                {ventasList.map((v) => (
          ...
          {totalVentasHoy > 0 && (
            <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-2.5 flex justify-between items-center">
              <span className="text-xs font-medium text-green-700">Total ventas hoy</span>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-gray-800 text-sm">💸 Gastos de hoy</h2>
          ...
          {totalGastosHoy > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-2.5 flex justify-between items-center">
              <span className="text-xs font-medium text-red-700">Total gastos hoy</span>
      ...
      {comprasList.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-2">
            <h2 className="font-semibold text-gray-800 text-sm">📦 Compras de hoy · {formatCLP(totalComprasHoy)}</h2>
