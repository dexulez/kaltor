...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase.from('repair_orders')
      .select('id, numero_ot, estado, created_at, customers(nombre), equipment(tipo_equipo, marca, modelo, falla_reportada)')
      .not('estado', 'in', '("entregado","cancelado","rechazado")')
                      ...
                      <div className="px-4 py-3 flex items-center justify-between hover:bg-gray-50 transition-colors">
                        <div className="min-w-0 flex-1">
                          ...
                          {ot.equipment?.falla_reportada && (
                            <p className="text-xs text-gray-500 truncate mt-0.5">🔧 {ot.equipment.falla_reportada}</p>
                          )}
                        </div>
