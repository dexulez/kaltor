...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase.from('repair_orders')
      .select('id, numero_ot, estado, falla_reportada, created_at, customers(nombre), equipment(tipo_equipo, marca, modelo)')
      .not('estado', 'in', '("entregado","cancelado","rechazado")')
  ...
  type OTItem     = { id: string; numero_ot: string; estado: string; falla_reportada?: string | null; created_at: string; customers: { nombre: string } | null; equipment: { tipo_equipo?: string | null; marca: string; modelo: string } | null }
                    ...
                    <Link key={ot.id} href={`/reparaciones/${ot.id}`}>
                      <div className="px-4 py-3 flex items-center justify-between hover:bg-gray-50 transition-colors">
                        <div className="min-w-0 flex-1">
                          ...
                          {ot.falla_reportada && (
                            <p className="text-xs text-gray-500 truncate mt-0.5">🔧 {ot.falla_reportada}</p>
                          )}
                        </div>
