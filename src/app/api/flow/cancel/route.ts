...
import { cancelSubscription } from '@/lib/flow/client'
...
export async function POST(req: NextRequest) {
  ...
  const { data: store } = await admin
    .from('stores')
    .select('id, flow_subscription_id, billing_status')
    .eq('id', profile.store_id)
    .single()

  if (!store?.flow_subscription_id) {
    return NextResponse.json({ error: 'No hay suscripción activa para cancelar' }, { status: 400 })
  }

  try {
    await cancelSubscription(store.flow_subscription_id as string)
    await admin.from('stores').update({ billing_status: 'cancelled' }).eq('id', store.id)
    return NextResponse.json({ ok: true })
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : 'Error al cancelar en Flow'
    console.error('[flow/cancel]', msg)
